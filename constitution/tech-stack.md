...
| 兼容性 | 部分确认 | Web 桌面布局最低支持宽度 960 CSS px、高度 600 CSS px；其他设备、系统和 API 兼容范围待确认 |
...
- 宽度和高度是独立下限，不代表固定分辨率组合；
- 支持范围内页面应连续自适应，不得依赖枚举大量固定宽高组合；
- 低于支持范围时仍须保证内容可滚动、可访问且不被裁切，但不要求保持完整桌面视觉比例；
- 布局验收以滚动归属、内容可访问性、无假滚动、无裁切和无双重滚动为核心；
- Browser E2E 使用少量代表性视口与连续尺寸调整进行验证，不将测试样本转化为新的产品断点。

## 已确认的分层本地验证策略

- 实施过程中使用直接覆盖改动的定向测试；
- 初次实施完成时运行静态检查、单元/集成测试、构建和相关 Browser E2E；
- 初次独立验收运行一次完整 Browser E2E，作为该轮 Spec 的完整浏览器回归；
- 小范围返工和复验只覆盖历史失败项与受影响回归，除非改动范围或证据表明必须再次完整回归；
