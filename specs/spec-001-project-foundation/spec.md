...
8. Pull Request 和 `main` 更新时自动执行的 CI；
...
- 在 Pull Request 和 `main` 分支更新时触发；
- 使用最小只读仓库权限；
- 使用 Node.js 24 和仓库固定的 pnpm 版本；
- 使用冻结锁文件安装依赖；
- 启动隔离的 PostgreSQL 18 和 Redis 8 测试服务；
- 依次执行格式检查、Lint、类型检查、自动化测试、前后端构建和 Docker 构建验证；
- 执行 Playwright 端到端测试；
- 对同一分支的新提交取消已经过时的进行中任务；
- 失败时保留足以诊断问题的测试报告；不得包含敏感信息；
- 不推送镜像、不连接服务器、不部署生产环境。

仓库分支保护属于 GitHub 外部配置，本 Spec 只提供可作为 Required Check 的 CI Job，不直接修改远端仓库设置。
...
- Pull Request 和 `main` 更新可以触发 CI；
- CI 执行本 Spec 规定的全部强制检查；
- CI 不发布镜像且不访问生产环境；
- CI 配置通过静态检查，并且在 GitHub 环境中有一次完整成功运行记录；若验收环境无法访问 GitHub，应将此项标记为 `BLOCKED`，不得以本地模拟替代远端运行结论。
...
- CI 完整 Docker 与 Playwright 验证会增加运行时间，应使用缓存和过时任务取消控制成本，但不得省略强制检查。
...
- 第一轮独立验收结论为 `BLOCKED`：AC-001 至 AC-008、AC-010、AC-011 均通过；AC-009 因 CI 工作流尚未推送至 GitHub、缺少远程成功运行记录而阻塞。
- 解除阻塞后应创建新的独立验收 Agent，仅复验 AC-009；在远程 CI 证据通过前不得将本 Spec 标记为 `Accepted`。
