...
- 每次任务开始时先判断任务类型和风险层级，再决定需要读取的代码范围、验证命令和是否需要 Browser E2E。
- 默认执行 `.ai/rules/testing.md` 的“最小充分验证”，不得把完整测试矩阵作为所有任务的固定模板。
- 文档、Spec 状态、验收记录和提交信息任务不启动 Node.js、Docker 或浏览器测试。
- 小型 Bug 默认只要求复现、定向测试和受影响回归；只有满足验证升级条件时才扩大到全量检查。
- 创建子 Agent 时，交接必须明确必跑项、明确不运行项及依据，避免子 Agent自行追加无关全量验证。
- 实施或验收 Agent 已对当前工作区状态提供可信测试证据后，协调开发 Agent 只检查报告与差异，不重复运行相同命令。
- 独立验收仍然必须独立形成证据，但同一验收阶段只运行一次规定层级的检查，不因等待或缺少新输出而从头重跑。
- 优先复用健康环境和现有依赖，不重复安装浏览器、Node.js、依赖或重新创建无必要的 Docker/临时环境。
- `1` 小时空转阈值只用于判断是否允许终止子 Agent，绝不作为任务最低执行时间或等待时间。
