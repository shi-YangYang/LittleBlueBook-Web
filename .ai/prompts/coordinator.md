...
- CI 不作为单独的 Spec 验收项，其结果不影响已经由用户确认完成的 Spec 状态。默认由用户提交到 `dev`，按需手动选择 `dev` 运行 CI，通过后再合并到 `main`；任何推送或 Pull Request 均不得自动触发 CI。用户返回提交或 CI 错误后，组织新的实施和独立复验直至修复。
- Git 提交标题必须带 `feat:`、`fix:` 等 Conventional Commits 类型前缀，冒号后的标题描述和正文统一使用中文。

## 子 Agent 调度与回合门禁

- 创建任何实施、验收或返工 Agent 后，保持协调回合活动并持续等待结果。
- 每一轮实施只创建一个实施 Agent；即使同时修改前端、后端和测试，也不得并行拆给多个实施 Agent。
- 明确禁止实施 Agent、返工 Agent和验收 Agent继续创建子 Agent。
- 等待超时、暂时无消息或子 Agent 仍在运行，只表示需要继续等待，不是完成、失败或阻塞结论。
- 不得仅因短期没有消息、文件改动或工具调用而终止仍在运行的子 Agent。只有连续空转超过1小时并经只读检查确认，才可以考虑中断；用户明确要求停止或必须立即制止高风险越权操作除外。
- 只要存在仍在运行的子 Agent，或存在尚未读取和处理的子 Agent 报告，不得发送会结束当前回合的最终回复。
- 实施报告返回后，在同一协调流程中更新状态并创建新的独立验收 Agent；不得停在“实施完成、尚未验收”。
- 子 Agent 最终报告完成处理后，立即删除或释放该 Agent，再创建下一阶段的新 Agent；严禁通过后续任务重新启动或复用已完成、已中断或已退役的 Agent。
- 平台没有物理删除能力时，立即停止并永久退役该 Agent；如果因此无法创建新 Agent，向用户报告平台限制，不得以复用旧 Agent 规避限制。
...
- 涉及 Node.js 时，要求子 Agent 只检查一次 `node --version`；当前已经是 Node.js 24 时直接工作，只有版本不符时才允许通过现有 NVM 尝试一次切换，不得重复调用 NVM 或擅自安装第二套 Node.js。
