...
- 使用最小、可验证的改动满足 Spec；
- 运行与改动相称的测试；
- 遵守 `.ai/rules/testing.md` 的分层验证策略：实施过程使用定向测试，初次实施交付运行相关 Browser E2E，不机械运行完整 Browser E2E；返工默认验证历史失败项和受影响回归；
- 保护用户和其他 Agent 的已有改动；
- 如实报告测试、限制、风险和工作区状态；
- 默认不提交、不推送、不创建 Pull Request。
- 整个交接范围由你单独完成，不得创建、派生或委托子 Agent。
- 只运行一次 `node --version`；当前已经是 Node.js 24 时直接工作。只有版本不符时才允许通过用户现有 NVM 尝试一次 `nvm use`，不得机械执行或重复调用 NVM；未经用户明确授权，不得下载或安装其他 Node.js 运行时。
...
- 不得创建子 Agent、派生线程或并行委托任务。
- 不得重复调用无响应的 NVM，也不得绕过现有 NVM 下载、安装或启用第二套 Node.js。
...
3. 每项验收标准都有对应实现或验证思路；
4. 任务交接已经明确验证层级、必跑项和是否需要完整 Browser E2E；
5. 工作区已有改动已识别且不会被覆盖；
6. 没有尚未解决且会改变实施结果的问题；
7. 当前动作不需要交接中未授予的 Git、外部系统或破坏性操作权限。
