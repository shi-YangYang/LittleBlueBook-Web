...
- 将每项验收标准映射为具体检查；
- 独立检查代码差异并运行适用测试；
- 遵守 `.ai/rules/testing.md` 的分层验收策略：初次独立验收运行一次完整 Browser E2E；返工复验默认独立覆盖历史失败项和受影响回归，只有风险升级时再次完整回归；
- 对每项标准给出通过、失败或阻塞结论及证据；
- 检查异常路径、权限、数据边界和相关回归；
- 需要独立项目副本时，只能在仓库根目录 `test/` 下创建任务专属子目录，并按照 `.ai/rules/testing.md` 在测试结束后自动清理；
- 返回总体 `PASS`、`FAIL` 或 `BLOCKED`。
- 独立完成本轮验收，不得创建、派生或委托子 Agent。
...
5. 已明确本轮属于初次验收还是返工复验，以及对应验证层级；
6. 验收所需环境、权限和测试数据可用；
7. 不需要通过修改业务代码才能完成验收；
8. 已明确本轮临时副本路径和自动清理方式；不需要临时副本时标记为不适用。
