...
所有强制验收项通过，且用户已明确确认验收完成。协调开发 Agent 应立即将 Spec 标记为 `Accepted`，然后提供提交信息，由用户自行提交。

## 状态推进与消息闭环

- `Implementing` 和 `Accepting` 是进行中状态，不是允许协调开发 Agent 结束回合的交付状态。
- 协调开发 Agent 创建子 Agent 后必须持续等待；单次等待超时或暂时无消息不得触发状态变化，也不得被解释为阻塞。
- 仍在运行的子 Agent 不得因短期无消息、无文件变化或未调用工具而被提前终止。连续空转超过1小时且经只读检查确认后，协调开发 Agent 才可以考虑中断；用户明确停止或必须立即制止高风险越权操作除外。
- 同一实施轮次只允许一个实施 Agent，复杂的前后端任务也不得并行拆给多个实施 Agent；实施 Agent 和验收 Agent 均不得创建自己的子 Agent。
- 子 Agent 返回结果后，协调开发 Agent 必须先将结果写入对应 Spec 记录并推进下一状态，再决定是否结束回合。
- 子 Agent 报告完成处理后必须立即删除或释放；平台不支持物理删除时立即停止并永久退役，后续不得复用。下一阶段必须创建新的 Agent。
- 实施完成必须继续进入独立验收；验收 `FAIL` 必须继续进入返工；返工完成必须继续进入新的独立复验。
- 独立验收 `PASS` 后，协调开发 Agent 应向用户汇总并请求最终确认；用户确认后必须立即进入 `Accepted`。
- 只有需要用户确认最新验收结果、已经进入 `Accepted`，或者进入已记录原因和解除条件且确实需要用户或外部动作的 `Blocked`，协调开发 Agent 才能结束当前协调回合。
- 回合意外中断后，恢复流程必须先核对既有子 Agent 状态和未处理结果，以仓库记录与实际 Agent 状态共同确定继续点。

## CI 与 Spec 完成状态

- CI 不单独设置为 Spec 验收项，也不是 Spec 从 `Accepting` 进入 `Accepted` 的门禁。
- Spec 是否完成只取决于已确认的非 CI 验收标准、独立验收结果和用户最终确认。
- 用户确认验收完成后，协调开发 Agent 先将 Spec 标记为 `Accepted`，再提供中文提交信息；默认由用户自行提交和推送。
- 用户将代码推送到 `dev` 后，按需手动选择 `dev` 运行 CI；任何分支推送或 Pull Request 都不自动触发 CI。CI 成功后由用户将 `dev` 合并到 `main`。
- CI 结果属于提交后反馈。CI 成功不需要再次改变 Spec 状态；CI 失败也不自动撤销或降级已经确认的 `Accepted` 状态。
- 用户返回提交或 CI 错误后，协调开发 Agent 应组织实施 Agent 修复，并创建新的独立验收 Agent 复验错误及相关回归，直至修复完成。
- 只有用户明确要求重新打开 Spec 时，提交后错误才改变已经确认的 Spec 状态。
